...
def main():
    ...
    artifactUrl="https://github.com/kapils-repos/Developer-Repo-New/tree/master/"+file
    print(files)
    ...
    if lineVal[1].split(':')[0] != "id":
        ...
        artifactTitle = lineVal[1].split(':')[1].strip().replace("\"", "")
        talendVersion = lineVal[2].split(':')[1].strip().replace("\"", "")
        artifactTags = lineVal[3].split(':')[1].strip().replace("\"", "")
        attrToManifest={}
        attrToManifest["artifactKey"]=artifactKey
        attrToManifest["artifactTitle"]=artifactTitle
        ...
        attrToManifest["artifactTags"]=artifactTags
        ...
        if(to != 'Travis CI'):
            upload_notification(to, artifact_title, artifact_id, artifact_url)
    else :
        ...
        if flag=="Y":
            ...
            to=userName
            artifact_title = lineVal[2].split(':')[1].strip().replace("\"", "")
            artifact_id = lineVal[1].split(':')[1].strip().replace("\"", "")
            artifact_url = artifactUrl
            if (to != 'Travis CI'):
                update_notification(to, artifact_title, artifact_id, artifact_url)
